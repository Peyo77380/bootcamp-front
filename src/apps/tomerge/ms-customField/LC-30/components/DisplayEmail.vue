  ...
  <v-layout row justify-center>
    <v-dialog v-model="dialog" fullscreen hide-overlay transition="dialog-bottom-transition">
      <v-card> 
        <div>
          <div class="mb-5">
            <v-toolbar dark color="warning" class="mb-6">
              ...
              <div class="table-responsive card-body"><h5 class="card-title">Information génerales</h5>
                <table class="align-middle mb-0 table table-borderless table-striped">
                  <tbody>
                    ...
                    <tr>
                        <td class="text-center font-weight-bold">Copie d'email</td>
                          <td class="text-center text-muted"> 
                            <span v-for="detail in details.emailCopy" :key="detail.id">
                              <br><b>{{detail.value }}</b>
                            </span>
                          </td> 
                    </tr>
                    <tr>
                        <td class="text-center font-weight-bold">Copie d'email caché</td>
                          <td class="text-center text-muted"> 
                            <span v-for="detail in details.hiddenCopie" :key="detail.id">
                              <br><b>{{detail.value }}</b>
                            </span>
                          </td> 
                    </tr>
                    <tr>
                        <td class="text-center font-weight-bold">Variable remplacée automatiquement</td>
                          <td class="text-center text-muted"> 
                            <span v-for="detail in details.variable" :key="detail.id">
                              <br>{{detail.value }}
                            </span>
                          </td> 
                    </tr>
