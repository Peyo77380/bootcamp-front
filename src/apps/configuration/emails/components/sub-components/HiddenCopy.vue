    ...
    <div>
        <div class="card-hover-shadow-2x card">
            <div class="card-header-tab card-header">
                ...
                >
                    <i
                        class="header-icon lnr-users icon-gradient bg-malibu-beach"
                    >
                    </i
                    >Adresse email en copie cachée
                </div>
            </div>
            <div class="scroll-area-sm">
                <div class="chat-wrapper p-1">
                    <ul class="todo-list-wrapper list-group list-group-flush">
                        <li
                            v-for="email in editedEmail.hiddenCopy"
                            :key="email.id"
                            ...
                            <div class="widget-content p-0">
                                <div class="widget-content-wrapper">
                                    ...
                                    <div class="widget-content-right ">
                                        ...
                                        >
                                            <i
                                                class="lnr-trash btn-icon-wrapper"
                                            ></i>
                                        </b-button>
                                    </div>
                                </div>
            ...
            <div class="d-block text-right card-footer">
                <button @click="openModalEmail" class="btn btn-primary">
                    Ajouter
                </button>
            </div>
        </div>
        <v-dialog v-model="modalHiddenEmail" persistent max-width="400">
            <v-card>
                <v-card-title class="headline"
                    >Ajouter adresse Email en copie cachée</v-card-title
                >
                <v-card-text>
                    <v-form>
    ...
    methods: {
        ...
        saveHiddenEmail() {
            ...
            if (this.editedIndex >= 0) {
                Object.assign(
                    this.editedEmail.hiddenCopy[this.editedIndex],
                    this.createdEmail
                ),
                    (this.editedIndex = -1);
            } else {
