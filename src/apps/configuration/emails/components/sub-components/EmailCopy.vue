    ...
    <div>
        <div class="card-hover-shadow-2x card">
            <div class="card-header-tab card-header">
                ...
                >
                    <i
                        class="header-icon lnr-user icon-gradient bg-malibu-beach"
                    >
                    </i
                    >Adresse email en copie
                </div>
            </div>
            <div class="scroll-area-sm">
                <div class="chat-wrapper p-1">
                    <ul class="todo-list-wrapper list-group list-group-flush">
                        <li
                            v-for="email in editedEmail.emailCopy"
                            :key="email.id"
        ...
        <v-dialog v-model="modalEmail" persistent max-width="400">
            <v-card>
                <v-card-title class="headline"
                    >Ajouter adresse Email</v-card-title
                >
                <v-card-text>
                    <v-form>
    ...
    methods: {
        ...
        saveEmail() {
            ...
            if (this.editedIndex >= 0) {
                Object.assign(
                    this.editedEmail.emailCopy[this.editedIndex],
                    this.createdEmail
                ),
                    (this.editedIndex = -1);
            } else {
